...
class User(Base):
    ...
    website = relationship(
        "WeddingWebsite", back_populates="user", uselist=False, cascade="all, delete"
    )
...
class WeddingWebsite(Base):
    ...
    guests = relationship(
        "Guest", back_populates="website", cascade="all, delete-orphan"
    )
...
class Guest(Base):
    ...
    rsvp_status = Column(
        SqlEnum(RSVPStatusEnum), default=RSVPStatusEnum.pending, nullable=False
    )
    attending = Column(
        SqlEnum(RSVPStatusEnum), default=RSVPStatusEnum.pending, nullable=False
    )
